...
class J1Route<T extends RouteConfig> {
  ...
  String build(T config) => _build(pathParamMap: config.pathParams, queryParamMap: config.queryParams);

  String _build({Map<String, Object> pathParamMap = const {}, Map<String, Object?> queryParamMap = const {}}) {
    ...
    return Uri(path: pathBuilder.toString(), queryParameters: queryMap.isNotEmpty ? queryMap : null).toString();
  }
}

String _buildPath({required List<PathComponent> components, bool useParams = false, Map<String, Object>? params}) {
  final pathBuilder = StringBuffer();
